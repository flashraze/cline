...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("bartek.claude-dev-dgits")
    ...
    **Note:** To ensure that the `bartek.claude-dev-dgits` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "bartek.claude-dev-dgits"
    ]
    ```
